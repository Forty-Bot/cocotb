...
class NVC(Simulator):
    ...
    def _build_command(self) -> List[Command]:
        ...
        if self.hdl_toplevel is not None:
            cmds[0] += ["-e", self.hdl_toplevel] + self._get_parameter_options(self.parameters)

        return cmds

    def _set_env(self) -> None:
        super()._set_env()
        self.env['LD_LIBRARY_PATH'] = cocotb.config.libs_dir

    def _test_command(self) -> List[Command]:

        if self.plusargs:
            raise ValueError(
                f"{type(self).__qualname__}: Simulator does not support plusargs"
            )
